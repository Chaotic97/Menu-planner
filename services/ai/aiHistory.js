 ...
 */
async function saveSnapshot(entityType, entityId, actionType, previousData) {
 ...
 */
async function getSnapshot(historyId) {
  const db = await getDb();
  const row = await db.prepare('SELECT * FROM ai_history WHERE id = ?').get(historyId);
  if (!row) return null;
 ...
 */
async function restoreSnapshot(historyId, broadcast) {
  const db = await getDb();
  const snapshot = await getSnapshot(historyId);
  if (!snapshot) {
  ...
  if (action_type === 'create') {
    ...
    if (entity_type === 'dish') {
      await db.prepare("UPDATE dishes SET deleted_at = NOW() WHERE id = ?").run(entity_id);
      if (broadcast) broadcast('dish_deleted', { id: entity_id });
    } else if (entity_type === 'menu') {
      await db.prepare("UPDATE menus SET deleted_at = NOW() WHERE id = ?").run(entity_id);
      if (broadcast) broadcast('menu_deleted', { id: entity_id });
    } else if (entity_type === 'task') {
      await db.prepare('DELETE FROM tasks WHERE id = ?').run(entity_id);
      if (broadcast) broadcast('task_deleted', { id: entity_id });
    } else if (entity_type === 'service_note') {
      await db.prepare('DELETE FROM service_notes WHERE id = ?').run(entity_id);
  ...
  if (action_type === 'delete' && previous_data) {
    ...
    if (entity_type === 'dish') {
      await db.prepare('UPDATE dishes SET deleted_at = NULL WHERE id = ?').run(entity_id);
      if (broadcast) broadcast('dish_created', { id: entity_id });
    } else if (entity_type === 'menu') {
      await db.prepare('UPDATE menus SET deleted_at = NULL WHERE id = ?').run(entity_id);
  ...
  if (action_type === 'update' && previous_data) {
    ...
    if (entity_type === 'dish') {
      await restoreDish(entity_id, previous_data);
      if (broadcast) broadcast('dish_updated', { id: entity_id });
    } else if (entity_type === 'menu') {
      await restoreMenu(entity_id, previous_data);
      if (broadcast) broadcast('menu_updated', { id: entity_id });
    } else if (entity_type === 'task') {
      await restoreTask(entity_id, previous_data);
 ...
 */
async function restoreDish(dishId, data) {
  const db = await getDb();
 ...
 */
async function restoreMenu(menuId, data) {
  const db = await getDb();
  if (data.name !== undefined) {
    await db.prepare(
 ...
 */
async function restoreTask(taskId, data) {
  const db = await getDb();
...
async function cleanupOldSnapshots() {
  const db = await getDb();
  const result = await db.prepare(
    "DELETE FROM ai_history WHERE created_at < NOW() - INTERVAL '1 day'"
  ).run();
