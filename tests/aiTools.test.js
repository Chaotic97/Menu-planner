...
describe('create_menu handler', () => {
  test('preview returns description with menu name', async () => {
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('create_menu', { name: 'Lunch Special' }, { preview: true });
    expect(result.description).toContain('Lunch Special');
    expect(result.message).toBeDefined();
  });

  test('preview includes event date when provided', async () => {
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('create_menu', { name: 'Wedding', event_date: '2026-06-15' }, { preview: true });
  ...
  test('execute creates a menu in the database', async () => {
    const { executeToolHandler } = getTools();
    const broadcasts = [];
    const broadcast = (type, payload) => broadcasts.push({ type, payload });

    const result = await executeToolHandler('create_menu', { name: 'AI Menu', description: 'Created by AI' }, { preview: false, broadcast });

    expect(result.success).toBe(true);
    expect(result.entityType).toBe('menu');
    expect(result.undoId).toBeDefined();

    const menu = await db.prepare('SELECT * FROM menus WHERE id = ?').get(result.entityId);
    expect(menu.name).toBe('AI Menu');
    expect(menu.menu_type).toBe('event');
    expect(broadcasts.some(b => b.type === 'menu_created')).toBe(true);
  });

  test('execute creates a menu with event_date', async () => {
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('create_menu',
  ...
  test('execute creates a standard (house) menu', async () => {
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('create_menu',
...
describe('create_dish handler', () => {
  test('preview includes dish name and category', async () => {
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('create_dish', { name: 'Risotto', category: 'main' }, { preview: true });
    expect(result.description).toContain('Risotto');
    expect(result.description).toContain('main');
  });

  test('execute creates a dish', async () => {
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('create_dish', { name: 'AI Risotto', category: 'starter' }, { preview: false });

    expect(result.success).toBe(true);
    const dish = await db.prepare('SELECT * FROM dishes WHERE id = ?').get(result.entityId);
...
describe('create_task handler', () => {
  test('preview shows task title', async () => {
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('create_task', { title: 'Order supplies' }, { preview: true });
    expect(result.description).toContain('Order supplies');
  });

  test('preview shows priority when non-default', async () => {
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('create_task', { title: 'Urgent thing', priority: 'high' }, { preview: true });
    expect(result.description).toContain('high');
  });

  test('execute creates a task with defaults', async () => {
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('create_task', { title: 'Test Task' }, { preview: false });

    expect(result.success).toBe(true);
    const task = await db.prepare('SELECT * FROM tasks WHERE id = ?').get(result.entityId);
...
describe('add_dish_to_menu handler', () => {
  ...
  beforeAll(async () => {
    const dishResult = await db.prepare('INSERT INTO dishes (name, description, category) VALUES (?, ?, ?)').run('Fuzzy Match Dish', '', 'main');
    testDishId = dishResult.lastInsertRowid;
    const menuResult = await db.prepare('INSERT INTO menus (name, description) VALUES (?, ?)').run('Fuzzy Match Menu', '');
    testMenuId = menuResult.lastInsertRowid;
  });

  test('resolves dish and menu by fuzzy name match', async () => {
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('add_dish_to_menu', { dish_name: 'Fuzzy Match', menu_name: 'Fuzzy Match' }, { preview: true });
    expect(result.description).toContain('Fuzzy Match Dish');
    expect(result.description).toContain('Fuzzy Match Menu');
  });

  test('returns error when dish not found', async () => {
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('add_dish_to_menu', { dish_name: 'Nonexistent Dish XYZ' }, { preview: true });
    expect(result.message).toMatch(/couldn.*find/i);
  });

  test('execute adds dish to menu', async () => {
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('add_dish_to_menu',
      { dish_id: testDishId, menu_id: testMenuId, servings: 3 },
      { preview: false, broadcast: () => {} }
    );

    expect(result.success).toBe(true);
    const link = await db.prepare('SELECT * FROM menu_dishes WHERE menu_id = ? AND dish_id = ?').get(testMenuId, testDishId);
    expect(link).toBeDefined();
    expect(link.servings).toBe(3);
  });

  test('rejects adding duplicate dish to menu', async () => {
    const { executeToolHandler } = getTools();
    // Already added above
    const result = await executeToolHandler('add_dish_to_menu',
...
describe('search_dishes handler', () => {
  test('finds matching dishes', async () => {
    await db.prepare('INSERT INTO dishes (name, description, category) VALUES (?, ?, ?)').run('Chocolate Cake', 'Rich dark chocolate', 'dessert');

    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('search_dishes', { query: 'Chocolate' }, { preview: true });
    expect(result.message).toContain('Chocolate Cake');
  });

  test('returns no results message for unmatched query', async () => {
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('search_dishes', { query: 'zzz_nonexistent_zzz' }, { preview: true });
...
describe('convert_units handler', () => {
  test('preview shows conversion description', async () => {
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('convert_units', { from_quantity: 250, from_unit: 'ml', to_unit: 'cups', ingredient_name: 'milk' }, { preview: true });
...
describe('add_service_note handler', () => {
  test('preview shows note title', async () => {
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('add_service_note', { title: 'VIP Alert', content: 'Peanut allergy table 5' }, { preview: true });
    expect(result.description).toContain('VIP Alert');
  });

  test('execute creates a service note', async () => {
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('add_service_note',
      { title: 'Staff Note', content: 'Short on parsley', shift: 'am' },
      { preview: false, broadcast: () => {} }
    );

    expect(result.success).toBe(true);
    const note = await db.prepare('SELECT * FROM service_notes WHERE id = ?').get(result.entityId);
...
describe('lookup_dish handler', () => {
  test('returns dish details by ID', async () => {
    const dish = await db.prepare('INSERT INTO dishes (name, description, category) VALUES (?, ?, ?)').run('Lookup Test', 'desc', 'main');
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('lookup_dish', { dish_id: dish.lastInsertRowid }, { preview: false });
    expect(result.message).toContain('Lookup Test');
    expect(result.message).toContain('main');
  });

  test('returns dish details by fuzzy name', async () => {
    await db.prepare('INSERT INTO dishes (name, description, category) VALUES (?, ?, ?)').run('Fuzzy Lookup Dish', '', 'dessert');
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('lookup_dish', { dish_name: 'Fuzzy Lookup' }, { preview: false });
    expect(result.message).toContain('Fuzzy Lookup Dish');
  });

  test('returns not found for missing dish', async () => {
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('lookup_dish', { dish_id: 99999 }, { preview: false });
...
describe('lookup_menu handler', () => {
  test('returns menu details by ID', async () => {
    const menu = await db.prepare('INSERT INTO menus (name, description) VALUES (?, ?)').run('Lookup Menu Test', 'A test');
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('lookup_menu', { menu_id: menu.lastInsertRowid }, { preview: false });
    expect(result.message).toContain('Lookup Menu Test');
  });

  test('includes event_date and menu_type in output', async () => {
...
describe('search_ingredients handler', () => {
  test('finds matching ingredients', async () => {
    await db.prepare('INSERT INTO ingredients (name, unit_cost, base_unit) VALUES (?, ?, ?) ON CONFLICT (name) DO NOTHING').run('Paprika', 2.50, 'kg');
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('search_ingredients', { query: 'Paprika' }, { preview: false });
    expect(result.message).toContain('Paprika');
  });

  test('returns no results for unmatched', async () => {
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('search_ingredients', { query: 'zzz_no_ingredient_zzz' }, { preview: false });
...
describe('search_tasks handler', () => {
  test('finds matching tasks', async () => {
    await db.prepare('INSERT INTO tasks (title, type, priority, source) VALUES (?, ?, ?, ?)').run('Dice onions', 'prep', 'high', 'manual');
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('search_tasks', { query: 'onions' }, { preview: false });
...
describe('get_system_summary handler', () => {
  test('returns summary stats', async () => {
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('get_system_summary', {}, { preview: false });
...
describe('unknown tool handler', () => {
  test('returns error message for unknown tool', async () => {
    const { executeToolHandler } = getTools();
    const result = await executeToolHandler('nonexistent_tool', {}, { preview: true });
